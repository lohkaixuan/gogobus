  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-hidden animate-modal-up">
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 w-10 h-10 bg-(--color-gray-light) rounded-full flex items-center justify-center hover:bg-gray-300 transition-colors z-10"
        >
          <X className="w-5 h-5" />
        </button>

        {/* Header */}
        <div className="bg-linear-to-r from-(--color-primary) to-(--color-primary-light) text-white px-6 py-6">
          ...
          {options.map((option) => (
            <div
              ...
              className={`relative rounded-2xl p-5 cursor-pointer transition-all duration-300 hover:scale-[1.02] ${
                option.type === 'health'
                  ? 'bg-linear-to-br from-(--color-primary) to-(--color-primary-light) text-white shadow-lg'
                  : 'bg-(--color-gray-light) hover:bg-gray-200'
              }`}
            >
              {/* Tag */}
              ...
              <div className="flex gap-4">
                <div className={`shrink-0 w-16 h-16 rounded-2xl flex items-center justify-center ${
                  ...
                  <div className={option.type === 'health' ? 'text-white' : 'text-(--color-primary)'}>
                    {getIcon(option.icon)}
                  </div>
                </div>
                ...
                <div className="flex-1">
                  ...
                  <p className={`text-sm mb-3 ${
                    option.type === 'health' ? 'text-white text-opacity-90' : 'text-(--color-text-secondary)'
                  }`}>
                    {option.description}
                  </p>
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    <div className="flex items-center gap-1.5">
                      <Clock className={`w-4 h-4 ${option.type === 'health' ? 'text-white' : 'text-(--color-text-secondary)'}`} />
                      <span>+{option.delay} min</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <Users className={`w-4 h-4 ${option.type === 'health' ? 'text-white' : 'text-(--color-text-secondary)'}`} />
              ...
              <div className="mt-4 pt-4 border-t border-white border-opacity-20">
                <div className="flex items-center justify-between text-xs mb-2">
                  <span className={`flex items-center gap-1 ${option.type === 'health' ? 'text-white text-opacity-90' : 'text-(--color-text-secondary)'}`}>
        ...
        {/* Info Footer */}
        <div className="bg-(--color-background) px-6 py-4 text-center">
